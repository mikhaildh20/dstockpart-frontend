...
export default function ModelPage(){
    ...
    const [dataModelsRaw, setDataModelsRaw] = useState([]);
    ...
    const loadData = useCallback(async (page, sort, cari, status) => {
        try{
            ...
            setDataModelsRaw(response.data.data || []);
    ...
    const handleAdd = useCallback(() => {
        router.push("/pages/model/add");
    }, [router]);

    const handleEdit = useCallback(
        (id) =>
        router.push(`/pages/model/edit/${encryptIdUrl(id)}`),
        [router]
    );
    ...
    const handleToggle = useCallback(
        ...
        const model = dataModelsRaw.find(item => item.Id === id);
        const isActive = model?.Status === 1;
        
        if (isActive) {
            const result = await SweetAlert({
                title: "Disable Model",
                text: "Are you sure you want to disable this model?",
                icon: "warning",
        ...
        try {
            ...
            Toast.success(data.message || "Model status updated successfully");
        ...
        [sortBy, search, sortStatus, loadData, dataModelsRaw]
    );
